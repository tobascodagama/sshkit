...
module SSHKit
  class TestCommand < UnitTest
    ...
    def test_working_as_a_given_user
      ...
      assert_equal "sudo -u anotheruser \"/usr/bin/env whoami\"", c.to_command
    end
    ...
    def test_working_as_a_given_user_and_group
      ...
      assert_equal "sudo -u anotheruser \"sg devvers -c \\\"/usr/bin/env whoami\\\"\"", c.to_command
    end
    ...
    def test_backgrounding_a_task_as_a_given_user
      ...
      assert_equal "sudo -u anotheruser \"nohup /usr/bin/env sleep 15 > /dev/null &\"", c.to_command
    end
    ...
    def test_backgrounding_a_task_as_a_given_user_with_env
      ...
      assert_equal "( A=b sudo -u anotheruser \"nohup /usr/bin/env sleep 15 > /dev/null &\" )", c.to_command
    end
    ...
    def test_umask_with_working_directory_and_user
      ...
      assert_equal "cd /var && umask 007 && sudo -u alice \"/usr/bin/env touch somefile\"", c.to_command
    end

    def test_umask_with_env_and_working_directory_and_user
      SSHKit.config.umask = '007'
      c = Command.new(:touch, 'somefile', user: 'bob', env: {a: 'b'}, in: '/var')
      assert_equal "cd /var && umask 007 && ( A=b sudo -u bob \"/usr/bin/env touch somefile\" )", c.to_command
    end
